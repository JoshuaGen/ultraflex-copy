...
Route::get('/gallery', function (Request $request) {
    ...
    // Discover location folders under public/Images/Gallery
    ...
    // If a location is selected via query string, list images in that folder
    $selectedSlug = $request->query('loc');
    $selected = null;
    $images = [];

    if ($selectedSlug && isset($slugToFolder[$selectedSlug])) {
        $selected = [
            'slug' => $selectedSlug,
            'name' => ucwords(str_replace(['-', '_'], ' ', $selectedSlug)),
        ];

        $folder = $slugToFolder[$selectedSlug];
        ...
        if (File::isDirectory($absDir)) {
            ...
            foreach ($files as $file) {
                ...
                $url = '/Images/Gallery/' . $encodePath($relFromGallery);
                $images[] = [
                    'id' => md5($relFromGallery),
                    'thumb' => $url,
                    'src' => $url,
                ];
            }
            usort($images, function ($a, $b) {
                return strcasecmp($a['src'], $b['src']);
            });
        }
    }
